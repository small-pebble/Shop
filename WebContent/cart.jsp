...
<html>
	<head>
		...
		<script type="text/javascript">
			function deleteCartItemByPid(pid){
				if(confirm("您是否要删除该项")){
					location.href("${pageContext.request.contextPath}/product?method=deleteCartItemByPid&pid="+pid);
				}
			}
			function claerCart(){
				if(confirm("您是否要清空购物车")){
					location.href("${pageContext.request.contextPath}/product?method=claerCart");
				}
			}
			...
			<div class="container">
							...
							<tbody>
								...
								<c:forEach items="${cart.cartItems }" var="entry">
									<tr class="active">
										...
										<td width="10%">
											<input type="text" name="buyNum" value="${entry.value.buyNum }" maxlength="4" size="10">
										</td>
										<td width="15%">
											<span class="subtotal">￥${entry.value.subTotal}</span>
										</td>
										<td>
											<a href="javascript:void(0);" class="delete" onclick="deleteCartItemByPid('${entry.value.product.pid}')">删除</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
				...
				<div style="margin-right:130px;">
					...
					<div style="text-align:right;margin-top:10px;margin-bottom:10px;">
						<a href="javascript:void(0);" onclick="claerCart()" id="clear" class="clear">清空购物车</a>
